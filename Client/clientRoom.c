...
void end(int n){
  ...
  char messageToSend[240] = "/leave";
  int sendM = send(dSR, messageToSend, sizeof(messageToSend), 0) ;
  if(sendM == -1){ 
...
void* thread_file_transfer_server_to_client(){
  ...
  aS.sin_port = htons(atoi(originalPort)+2);
...
void* thread_file_transfer_client_to_server(void* args){
  ...
  aS.sin_family = AF_INET;
  inet_pton(AF_INET,ip_adress,&(aS.sin_addr)) ;
  aS.sin_port = htons(atoi(originalPort)+1);
  socklen_t lgA = sizeof(struct sockaddr_in) ;
  ...
  while(!feof(fps)){
    ...
    if (send(dSFTCTS, buffer, sizeof(buffer), 0) == -1) {
...
void* thread_send_room(){

  char messageToSend[240];
  while(1){
    if(fgets(messageToSend,sizeof(messageToSend),stdin) == NULL){
      perror("Erreur du fgets");
      exit(0);
    }
    ...
    *pos = '\0';
    ...
    int sendPseudo = send(dSR, messageToSend, sizeof(messageToSend), 0) ;
    if(sendPseudo == -1){ 
      perror("Problème avec Send");
      exit(-1);
    }
    ...
    /*---------------------------------------JOIN FUNCTIONALITY-----------------------------*/
    if(strcmp(messageToSend,"/leave") == 0){
      pthread_exit(NULL);
    }
  }
}
...
//----------------------------------THREAD RECEIVE ROOM-----------------------
void* thread_receive_room(){
  char msgRec[240];
  while(1){
    int recvM = recv(dSR, msgRec,sizeof(msgRec),0) ;
    if(recvM == 0){
      printf("Salon éteint\n");
      close(dSR);
      exit(-1);
...
int main(int argc, char *argv[]){
  ...
  if(atoi(argv[4]) < 0){
    printf("Le numéro de port original %s un invalide (premier argument)\n",argv[4]);
    exit(-1);
  }
  DIR* dir;
  ...
  dSR = socket(PF_INET, SOCK_STREAM, 0);
  if (dSR == -1){
    perror("Problème pendant la création de la socket");
    exit(-1);
  }
  ...
  int connectS = connect(dSR, (struct sockaddr *) &aS, lgA) ;
  if (connectS == -1){
    perror("Problème pendant l'envoi de la demande de connexion");
    exit(-1);
  }
/*-------------------------------Création du thread Receive-------------------------*/
  int thread_recep = pthread_create(&thread_idRec,NULL,thread_receive_room,NULL);
  if(thread_recep != 0){
    perror("Erreur thread réception");
    exit(-1);
  }
/*-------------------------------Création du thread Send----------------------------*/
  int thread_saisie = pthread_create(&thread_idSend,NULL,thread_send_room,NULL);
  if(thread_saisie != 0){
